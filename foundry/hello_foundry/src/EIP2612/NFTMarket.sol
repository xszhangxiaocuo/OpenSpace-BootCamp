...
contract NFTMarket {
  ...
  function permitBuy(PermitData calldata permitData,TokenPermitData calldata tokenPermitData, Signature[] calldata signature) public {
    ...
    require(permitData.nonce == nonces[permitData.buyer], "Invalid nonce");
    Signature memory buySig = signature[0];
    bytes32 hashStruct = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, keccak256(abi.encode(PERMIT_TYPEHASH, permitData.buyer, permitData.tokenId, permitData.amount,_useNonce(permitData.buyer), permitData.deadline))));
    address signer = ecrecover(hashStruct, buySig.v, buySig.r, buySig.s);
    require(signer == _owner, "Invalid signature"); // 签名地址必须是合约部署者
