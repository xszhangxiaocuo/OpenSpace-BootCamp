...
contract AirdropMerkleNFTMarket is MerkleDistributor, Multicall {
  ...
  constructor(address _tokenAddress, address _nftAddress, bytes32 _merkleRoot) MerkleDistributor(_merkleRoot) {
    token = _tokenAddress;
  ...
  // 通过permit授权
  function permitPrePay(TokenPermitData memory tokenPermitData, Signature memory signature) public {
    IERC20Permit(token).permit(tokenPermitData.owner, tokenPermitData.spender, tokenPermitData.amount, tokenPermitData.deadline, signature.v, signature.r, signature.s);
  }
