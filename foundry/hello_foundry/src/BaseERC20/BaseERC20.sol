...
contract BaseERC20 {
  ...
  modifier enoughBalance(address _addr, uint256 _value) {
    require(balances[_addr] >= _value, "ERC20: transfer amount exceeds balance");
    _;
  }

  // 校验代取额度
  modifier enoughAllowances(address _from, address _to, uint256 _value) {
    require(allowances[_from][_to] >= _value, "ERC20: transfer amount exceeds allowance");
    _;
  }
  ...
  // 转账方法
  function transfer(address _to, uint256 _value) public noneZeroAdress(_to) enoughBalance(msg.sender, _value) returns (bool) {
  ...
  function approve(address _spender, uint256 _value) public noneZeroAdress(_spender) returns (bool) {
  ...
  function transferWithCallback(address _to, uint256 _value, bytes calldata _data) public noneZeroAdress(_to) enoughBalance(msg.sender, _value) returns (bool) {
